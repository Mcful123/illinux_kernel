...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        boot = (bootb_t*)mod->mod_start; // load the boot block for file system
    ...
    init_rtc(); //enable interrupts from the rtc
    
     //enable interrupts from the pit

    init_keyboard(); //enable interrupts from the keyboard
    ...
    int i;
    for(i = 0; i < 3; i++){
        memset(terminals[i].top_line_buffer, 0, 160);
        memset(terminals[i].keyboard_buffer, 0, 128);
        terminals[i].buffer_length = 0;
        terminals[i].keyboard_lock = 0;
